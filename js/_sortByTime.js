...
let listOfAttractions = XHRcalls.getTimeData();

console.log('list of attractions', listOfAttractions);

let attractionItem0 = listOfAttractions.attractions[3]; // output the object with the id of 4/ne Emporium
...
let attractionTimeValues = listOfAttractions.attractions[3].times; // outputs ["9:00", "11:00"];
...
for (let i = 0; i < attractionsWithTimes.length ; i++) {
    ...
    for(let j = 0; j < listOfAttractions.attractions.length; j++) { //loops through the list of attractions 
        let currentAttractionsItem = listOfAttractions.attractions[j]; // sets the current attractions as a value of this variable
